...
document.getElementById('loginForm').addEventListener('submit', function(e) {
    ...
    ajaxPost('/auth/login', { username, password }, function(response) {
        if (response.code === 200) {
            ...
            if (response.data.role === "DOCTOR" || response.data.role === "ROLE_DOCTOR") {
...
function loadHealthProfile() {
    ajaxGet('/profile/my', function(res) {
        if (res.code!==200) { showMessage(res.message,'error'); return; }
        const p = res.data;
        const exportBtn = document.getElementById('exportProfileBtnContainer');
        if (exportBtn) {
            exportBtn.style.display = currentUser && currentUser.role === 'PATIENT' ? 'block' : 'none';
        }
...
// ==========================================
// 健康档案一键下载功能 (AJAX Blob 处理)
// 限定只有角色为患者才能看到和点击
// ==========================================
function downloadHealthProfile() {
    // 逆向校验 1：如果你不是患者，直接拒绝发请求
    const userRole = currentUser ? currentUser.role : localStorage.getItem('role');
    if (userRole !== 'PATIENT') {
        showMessage('仅患者可下载个人健康档案', 'error');
        return;
    }
    ...
    .then(response => {
        // 逆向情况捕获：后端根据你的逻辑返回的所有异常状态码
        if (response.status === 401) throw new Error("认证失效，请重新登录。");
        if (response.status === 403) throw new Error("越权操作：只有患者具备下载权限。");
        if (response.status === 400) throw new Error("用户状态异常或账号被冻结。");
        if (!response.ok) throw new Error("服务器生成文件时发生未知错误。");
        
        // 走到这就说明全是正确的了，剥离二进制大对象 Blob
    ...
    .then(blob => {
        ...
        a.download = `我的芯芯医疗健康档案_${dateStr}.md`;
        ...
        showMessage('✅ 档案下载成功，请保护好您的隐私。', 'success');
    })
    .catch(error => {
        console.error('Export Error:', error);
        showMessage('❌ 档案生成失败：' + error.message, 'error');
    })
    .finally(() => {
        // 恢复按钮状态
        if (btn) {
            btn.disabled = false;
            btn.innerHTML = '⬇️ 一键下载健康档案 (MD版)';
        }
    });
}
