    ...
    ajaxGet(url, function(res) {
        if (res.code === 200 && res.data) {
            ...
            schedules.forEach(item => {
                ...
                tr.innerHTML = `
                    <td style="font-weight: 500">${item.scheduleDate}</td>
                    <td><span class="badge ${timeSlotBadge}">${timeSlotText}</span></td>
                    <td><span style="color:#64748b">${item.totalLimit}</span></td>
                    <td><span style="color:#0f172a; font-weight: 500">${item.bookedCount}</span></td>
                    <td>${statusHtml}</td>
                `;
