...
~~~
如果您觉得合适，请点一下小心心，本项目可自行下载；
已附带数据库sql脚本，可以快速部署启动项目；
当然各位技术大佬觉得有什么问题可以批评指出；
咱们一起做大做强，哈哈哈；
~~~

---
## 主页面
![111.png](src/main/resources/images/111.png)
---
## 患者页面
![222.png](src/main/resources/images/222.png)
---
## 医生页面
![333.png](src/main/resources/images/333.png)
---
## 当前优化结论

截至 2026-05-27，本项目已完成一次 API 与前端联调检查。整体架构可运行，`mvn -DskipTests compile` 编译通过；本次主要修复了医生工作台、在线问诊医生端、预约操作权限与文档滞后的问题。

已处理的优化点：
- 医生工作台聚合接口按当前登录医生用户映射到 doctor 表 ID，修正排班/问诊统计查不到数据的问题。
- 医生端问诊页面对齐后端实际接口：`/message`、`/accept`、`/finish`，并统一使用数字状态码。
- 预约创建以排班记录为准校验医生，取消/详情/改签增加当前用户归属校验，避免越权操作。
- 改签只能选择同一医生的可用号源，避免跨医生改签造成数据不一致。
- 清理 `.orig/.rej` 补丁残留文件，并移除 `doctor.html` 文件末尾空字节。

后续可继续增强但不影响课程演示的点：
- 增加单元测试/接口测试覆盖核心业务分支。
- 将生产环境数据库密码、JWT secret 改为环境变量注入。
- 将医生端处方开具入口补齐为完整表单，而不是仅结束问诊。
...
│       ├── DoctorWorkspaceController.java # /api/doctor-workspace/*
│       ├── LabReportController.java     # /api/reports/lab/*
│       ├── ImagingReportController.java # /api/reports/imaging/*
│       ├── HealthProfileController.java # /api/profile/*
│       ├── NotificationController.java  # /api/notifications/*
│       └── ServiceController.java       # /api/service/*
│
├── src/main/resources/
│   ├── application.yml              # 应用配置（数据库/JWT/MyBatis）
│   ├── init.sql                     # 数据库初始化（16张表 + 测试数据）
│   └── static/
│       ├── index.html               # 患者端单页应用主页面
│       ├── doctor.html              # 医生工作台页面
...
| 功能 | 说明 |
|------|------|
| 医生工作台 ✨ | 医生登录后查看个人信息、今日问诊统计、排班列表并处理患者问诊 |
...
### 5. 访问系统
浏览器打开：http://localhost:4060

医生工作台：http://localhost:4060/doctor.html
...
### v3.1.0 (2026-05-27)
- 🔧 **接口联调修复**
  - 修复医生工作台聚合 SQL：医生用户 ID 正确映射到 doctor.id，问诊状态按数字码统计。
  - 修复医生端问诊页面接口路径与字段名：接诊、发消息、结束问诊均对齐后端 Controller。
  - 修复预约创建以排班医生为准，避免前端传入 doctorId 与 scheduleId 不一致。
- 🔒 **权限优化**
  - 预约详情、取消、改签增加当前患者归属校验。
  - 问诊详情和消息轮询增加参与方校验，非患者/接诊医生不能访问。
- 📄 **文档维护**
  - 更新 Controller/Service/Mapper/Entity 数量、运行端口和医生工作台说明。
  - 清理补丁残留文件和页面空字节。
